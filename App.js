...
export default class App extends React.Component {
  ...
  getTodos() {
    ...
    return axios.get('http://192.168.1.105:3009/api/todos')
      .then(response => {
  ...
  handlePressEdit() {
    axios.put(`http://192.168.1.105:3009/api/todos/${this.state.editId}`, {
    ...
    axios.delete(`http://192.168.1.105:3009/api/todos/${this.state.editId}`,{
		id:this.state.id
    })
      .then(response => {
  ...
  handlePressAdd() {
    ...
    axios.post('http://192.168.1.105:3009/api/todos', payload)
      .then(response => {
        ToastAndroid.show(response.data.message, ToastAndroid.SHORT);
      ...
      <View>
        <Modal
          ...
          <View>
            ...
            <Button onPress={() => this.setState({ modalVisible: false, mode: 'add', descriptionInput: '', titleInput: '', editId: '' })} title="Close" buttonStyle={{ marginBottom: 5 }} />
			<Button onPress={this.handlePressDelete} title="Delete"  backgroundColor="red"/>
          </View>
        </Modal>
